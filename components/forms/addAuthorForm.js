...
const addAuthorForm = (obj = {}) => {
  clearDom();
  const domString = `
    <form id="${obj.firebaseKey ? `update-book--${obj.firebaseKey}` : 'submit-author'}" class="mb-4">
      ...
      <div class="form-group">
        ...
        <input type="email" class="form-control" id="email" aria-describedby="Email" value="${obj.email || ''}" placeholder="Enter Email" required>
      </div>
